...
#endif
{
	...
	if (DEBUG_MESSAGE)
		SLOGD("putYUV420toJpegMem: destImage(%p), imageSize(%d), inputImage(%p,%p,%p), w(%d), h(%d), q(%d)",
                destImage, imageSize, inputYImage, inputCbImage, inputCrImage, width, height, quality);
...
#warning using JPEG_LIB_VERSION >= 70
	...
	for (j = 0; j < height; j += 16) {
		for (i = 0; i < 16; i++) {
			y[i] = (JSAMPLE *)(inputYImage + width * (i + j));

			if (i % 2 == 0) {
				cb[i/2] = (JSAMPLE *)(inputCbImage + ((width / 2) * ((i + j) / 2)));
				cr[i/2] = (JSAMPLE *)(inputCrImage + + ((width / 2) * ((i + j) / 2)));
			}
		}
 ...
 */
// psw0523 add struct for planar y, cb, cr
struct ycbcr_planar {
    unsigned char *y;
    unsigned char *cb;
    unsigned char *cr;
...
static void jpeg_encode_yuv420(struct convert_context *d, unsigned char *src, unsigned char *dst) {
	...
	jpeg_start_compress(cinfo, TRUE );
    // psw0523 fix for planar y, cb, cr address
#if 0
