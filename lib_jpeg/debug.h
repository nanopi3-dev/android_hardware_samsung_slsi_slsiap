...
#define info(format, ...) do { SLOGD ("[ %s:%d ] " format, __FILE__, __LINE__, ## __VA_ARGS__);\
	} while(0)
...
#ifdef DEBUG
#define dprint(source, format, ...) do {if(((source) & (LOG_SOURCE))) {\
	SLOGD ("[%s:%d] " format, __FILE__, __LINE__, ## __VA_ARGS__); fflush(stderr);} } while(0)
...
#define XMALLOC(var, type, size)	\
	do { \
		var = (type) malloc(size); \
		if (!var) {SLOGE("[%s:%d %s] MEMALLOC: OUT OF MEMORY !!! Cant allocate %lu bytes.\n", __FILE__, __LINE__, __PRETTY_FUNCTION__, (long unsigned int) size);} \
		else { CLEAR(*var);}\
	} while (0)
