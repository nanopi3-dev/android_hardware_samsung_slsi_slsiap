...
#define MIXER_XML_PATH		"/system/etc"
#define	MIXER_XML_NAME		"tiny_hw"
#define	MIXER_MAX_CARD		3
                      ...
                      const XML_Char **attr)
{
	...
	LOGD("%s:%d tag=%s\n", __FUNCTION__, __LINE__, tag_name);
...
struct audio_route *audio_route_init(unsigned int card, const char *xml_path)
{
    ...
    property_get("ro.product.device", prop, NULL);
	snprintf(name, PATH_MAX, "%s/%s", MIXER_XML_PATH, MIXER_XML_NAME);
	snprintf(path, PATH_MAX, "%s.%s.xml", name, prop);

	/* find at /system/etc */
	if (access(name, R_OK) != 0) {
		for (i=0 ; i<MIXER_MAX_CARD; i++) {
			/*
			 * find default
			 * tiny_hw.xml
			 */
			LOGD("+ %s path %s", __FUNCTION__, path);
			if (i == 0 && access(path, R_OK) == 0) break;

			/*
			 * find card depend file
			 * tiny_hw.0.xml, tiny_hw.1.xml....
			 */
			snprintf(path, PATH_MAX, "%s.%d.%s.xml", name, i, prop);
			LOGD("- %s path %s", __FUNCTION__, path);
	...
	card = i;
	ALOGI("Reading configuration from %s sound card %d\n", path, card);
